...
namespace V_AnimationSystem {
    ...
    public class V_UnitSkeleton {
        ...
        public Vector3 GetBodyPartPosition(string bodyPartName) {
            Vector3 pos = GetSkeletonUpdater().GetAnimWithBodyPartName(bodyPartName).currentAnimFrame.pos;
            return ConvertLocalPositionToWorldPosition(pos);
        }
        public void ReplaceBodyPartSkeletonAnim(string bodyPartName, UnitAnim unitAnim) {
            ReplaceBodyPartSkeletonAnim(bodyPartName, unitAnim.GetSkeletonAnim_BodyPartCustom(bodyPartName));
        }
